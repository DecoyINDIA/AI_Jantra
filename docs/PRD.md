...
- **Phase 1 — First Ops agent, dogfooded on Xolver (MVP).** Design partner #1 is Xolver itself. Multi-turn (R6), persistence (R7), scheduling/triggers (R8), the integrations the chosen Xolver process needs (subset of I1–I4), Slack approvals + handoff (A1–A2), one real Ops agent (O1–O3, chosen process) live on Xolver's own operations, fully audited. **Exit: an Ops agent doing real work in production at Xolver, with a clean audit trail we can show prospects.**
