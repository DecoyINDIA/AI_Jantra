# Mainframe — agent runtime

Mainframe builds autonomous agents that run the parts of a business no one wants to babysit. This repo is the **product**, not the marketing site: a reusable TypeScript runtime for those agents, plus the first reference agent (customer support) built on it.
...
- **Audit everything.** The brand promise is "you can see everything it did, and why." Thinking, messages, tool calls, policy decisions, approvals, results, and handoffs are all written to `.mainframe/audit/<run>.jsonl`.
